...
CSV_FILE = os.path.join(BASE_DIR, config["CSV_FILE"])
...
def extract():
    ...
    urls = {
        "Bán": "https://batdongsan.com.vn/nha-dat-ban",
        "Cho thuê": "https://batdongsan.com.vn/nha-dat-cho-thue"
    }

    for loai, base_url in urls.items():
        logger.info(f"\n=== Bắt đầu crawl loại: {loai} ===")
        links = crawl_all_listing_urls(base_url, pages_start = 244   , pages_end=  254   )
        new_links = filter_new_links(links)
        logger.info(f"Tổng: {len(links)} | Mới: {len(new_links)} bài đăng")
        ...
        save_to_csv(data, file_path=f"../data_raw/gia_nha.csv")
